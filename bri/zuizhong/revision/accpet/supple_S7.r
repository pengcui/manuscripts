####################################################################################
Online Supporting Information S7.

The related R codes are available.
###################################################################################


# We highly recommend to run these codes using Rstudio piece by piece.
# 1. The support vector regression model and plotting.
...
data=read.table('./max_sig.txt',header=T)  ## modify this line to choose avg_sig/max_sig
...
title(ylab="The Dnase-seq signals (log2)", xlab="Predicted signals (log2)",mgp=c(2.3,1,0))
mylm = lm(mylist[[2]]~mylist[[1]])
abline(mylm, lwd=2)
summary(mylm)
text(-0.5,-4.5,bquote(R^2 ~ '=' ~ 0.66),cex=0.8)
...
# TF barplot
barplot(featurepowers[tfvec,2],ylim=c(0,0.7),xlab='',ylab='',axisnames=F)
mtext(featurepowers[tfvec,1],line=0.5,side=1,at=seq(0.7,28,1.2),las=2,cex=0.8)
# -------------------------------------

mtext('R',side=2,line=2.3)
...
data=read.table('./avg_sig.txt',header=T)  ## modify this line to choose avg_sig/max_sig
...
# 3. compute coefficients of determination for all possible one-feature, two-feature, 
...
# So we use 'snowfall' package to parallel this task.
...
svrcrossvalidation=function(featurevector){   
  ...
  myresult=round(cor(actual,pre),3) ##  compute the sqrt of coefficient of determination
  return(myresult)
}
...
combs=combs+1  ## because the first column is dnase1sig 
mylist=list()
...
# (3) violin plot of feature combination results
